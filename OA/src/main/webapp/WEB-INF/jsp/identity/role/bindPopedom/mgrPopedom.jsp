	    ...
	    $.ajax({
				   ...
				   success:function(result){ //后台响应成功时的回调函数
					  $.each(result,function(){
						  //我们只需要展示一级 二级模块即可，因此code.length为12 的直接过滤
						  if(this.code.length != 12){
							//d.add(id,父级节点的id,节点名字,请求地址,标题,跳转位置);
								d.add(this.code,this.pcode,this.name,this.code.length == 8 ? '/identity/popedom/loadThirdModule?code='+this.code+"&id=${id}" : "javascript:void()",this.name,"rightFrame");
						  }
